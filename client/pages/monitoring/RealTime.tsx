...
function Content() {
  ...
  useEffect(() => {
    const u = onSnapshot(collection(db, "geo_tracking"), (s) =>
      setTracks(s.docs.map((d) => d.data())),
    );
    return () => u();
  }, []);
    ...
    <section className="container py-10">
      ...
      <div className="mt-6 grid md:grid-cols-2 gap-6">
        {tracks.map((t) => (
          <div
            key={t.trackingId}
            className="rounded-xl border p-4 bg-white/70 dark:bg-black/30"
          >
